...
def plot_distributed_array(arr: DistributedArray):
    ...
    dist_array = DistributedArray(global_shape=arr.global_shape, base_comm=arr.base_comm,
    ...
    if arr.rank == 0:
        figure, (ax1, ax2) = plt.subplots(nrows=1, ncols=2, figsize=(18, 5))
        ax1.matshow(full_arr, cmap='rainbow')
        ax1.set_title(f"Original Array")
