...
class MPIHalo(MPILinearOperator):
    def __init__(
        ...
        self.ndim = len(dims)
        self.edge = edge
        self.halo = self._parse_halo(halo, comm, edge=edge)
        self.scatter_type = scatter
        self.boundary_mode = boundary_mode
        self.comm  = comm
        self.dtype = dtype

        self.halos = comm.allgather(self.halo)
    ...
    def _parse_halo(self, h, comm, edge=False):
        if edge:
    ...
    def _calc_global_extent(self):
        ...
        for ax, gdim in enumerate(self.global_dims):
            minus_halo = np.sum([h[2 * ax] for h in self.halos])
            plus_halo = np.sum([h[2 * ax + 1] for h in self.halos])
            ext.append(gdim + minus_halo + plus_halo)
        return tuple(ext)
    ...
    def _matvec(self, x):
        ...
        y = DistributedArray(global_shape=self.shape[0],
                             local_shapes=self.comm.allgather(self.local_extent),
