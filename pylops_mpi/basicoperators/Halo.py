...
class MPIHalo(MPILinearOperator):
    def __init__(
        ...
        self.shape = (int(np.sum(self._local_extent_sizes)), int(np.prod(self.global_dims)))
    ...
    def _parse_halo(self, h):
        if not isinstance(h, int):
            h = tuple(h)
            if len(h) == 1:                halo = h * (2 * self.ndim)
            elif len(h) == self.ndim:      halo = sum(tuple([(d, d) for d in h]), ())
            elif len(h) == 2 * self.ndim:  halo = h
            else: raise ValueError(f"Invalid halo length {len(h)} for ndim={self.ndim}")
            return halo
        else:
            halo = (h,) * (2 * self.ndim)
            trimmed = list(halo)
    ...
    def _build_topo(self):
        cart_comm = self.comm.Create_cart(self.proc_grid_shape, periods=[False] * self.ndim, reorder=True)
        neigh = {}
        for ax in range(self.ndim):
    ...
    def _exchange_adjoint_along_axis(self, ncp, arr, axis, before, after):
        minus_nbr, plus_nbr = self.neigh[('-', axis)], self.neigh[('+', axis)]
        out = arr.copy()
        if before == 0 and after == 0: return out
        ...
        if halo_minus: out[halo_minus] = 0
        if halo_plus : out[halo_plus]  = 0
        ...
        if core_minus: out[core_minus] += rcv
        ...
        if core_plus: out[core_plus] += rcv
        return out
