...
nx = 150
# Size of y at each rank
ny_i = 25
# Size of y at all ranks
ny = MPI.COMM_WORLD.allreduce(ny_i)
nz = 95
x = 800 + 4 * np.arange(nx)
x = x / 1000.0
z = 800 + 4 * np.arange(nz)
z = z / 1000.0
m = np.log((rank + 1) * np.random.uniform(1.8, 3.8, (nx, nz)))
y_i = (rank + 1) * np.arange(ny_i)
# Extending over first axis
m3d = np.tile(m[np.newaxis, :, :], (ny_i, 1, 1))

# Smooth model
nsmoothx, nsmoothz = 30, 20
mback = filtfilt(np.ones(nsmoothx) / float(nsmoothx), 1, m, axis=0)
mback = filtfilt(np.ones(nsmoothz) / float(nsmoothz), 1, mback, axis=1)
# Extending over first axis
...
d_dense = d_dense_dist.asarray().reshape((ny, nx, nz))
d_dense_adj = d_dense_adj_dist.asarray().reshape((ny, nx, nz))

if rank == 0:
    fig, axs = plt.subplots(nrows=1, ncols=4, figsize=(15, 9))
    axs[0].imshow(d_dense[7, :, :], cmap="gray", vmin=d_dense.min(), vmax=d_dense.max(), extent=(x[0], x[-1], z[-1], z[0]))
    axs[0].set_title("Data")
    axs[0].axis("tight")

    axs[1].imshow(d_dense_adj[7, :, :], cmap="gray", vmin=d_dense_adj.min(), vmax=d_dense_adj.max(), extent=(x[0], x[-1], z[-1], z[0]))
    axs[1].set_title("Adjoint")
    axs[1].axis("tight")

    axs[2].imshow(m3d[7, :, :], cmap="gist_rainbow", vmin=m.min(), vmax=m.max(), extent=(x[0], x[-1], z[-1], z[0]))
    axs[2].set_title("Model")
    axs[2].axis("tight")

    axs[3].imshow(mback3d[7, :, :], cmap="gist_rainbow", vmin=m.min(), vmax=m.max(), extent=(x[0], x[-1], z[-1], z[0]))
