...
m3d_dist = pylops_mpi.DistributedArray(
    global_shape=ny * nx * nz, base_comm_nccl=nccl_comm, engine="cupy"
)
m3d_dist[:] = cp.asarray(m3d_i.flatten())

# Do the same thing for smooth model
mback3d_dist = pylops_mpi.DistributedArray(
    global_shape=ny * nx * nz, base_comm_nccl=nccl_comm, engine="cupy"
)
...
d0_dist = pylops_mpi.DistributedArray(
    global_shape=ny * nx * nz, base_comm_nccl=nccl_comm, engine="cupy"
)
d0_dist[:] = 0.0
...
minv3d_reg_dist = pylops_mpi.optimization.basic.cgls(
    StackOp, dstack_dist, x0=mback3d_dist, niter=100, show=True
)[0]
...
###############################################################################
# TV-Regularized inversion with PD
...
tau = 1.0
mu = 0.99 / (tau * L)
minv3d_pd_dist = pylops_mpi.proximal.optimization.primaldual.PrimalDual(
    l2d,
    l1d,
    Gopd,
    x0=mback3d_dist,
...
minv3d_pd = minv3d_pd_dist.asarray().reshape((ny, nx, nz))
...
if rank == 0:
    ...
    axs[3][0].imshow(
        minv3d_iter[5, :, :].T.get(), cmap="gist_rainbow", vmin=m.min(), vmax=m.max()
    )
    axs[3][0].set_title("Inverted Model iter x-z")
    axs[3][0].axis("tight")
    axs[3][1].imshow(
        minv3d_iter[:, 200, :].T.get(), cmap="gist_rainbow", vmin=m.min(), vmax=m.max()
    )
    axs[3][1].set_title("Inverted Model iter y-z")
    axs[3][1].axis("tight")
    axs[3][2].imshow(
        minv3d_iter[:, :, 220].T.get(), cmap="gist_rainbow", vmin=m.min(), vmax=m.max()
    )
    axs[3][2].set_title("Inverted Model iter x-y")
    axs[3][2].axis("tight")

    axs[4][0].imshow(
        minv3d_ne[5, :, :].T.get(), cmap="gist_rainbow", vmin=m.min(), vmax=m.max()
    )
    axs[4][0].set_title("Normal Equations Inverted Model iter x-z")
    axs[4][0].axis("tight")
    axs[4][1].imshow(
        minv3d_ne[:, 200, :].T.get(), cmap="gist_rainbow", vmin=m.min(), vmax=m.max()
    )
    axs[4][1].set_title("Normal Equations Inverted Model iter y-z")
    axs[4][1].axis("tight")
    axs[4][2].imshow(
        minv3d_ne[:, :, 220].T.get(), cmap="gist_rainbow", vmin=m.min(), vmax=m.max()
    )
    axs[4][2].set_title("Normal Equations Inverted Model iter x-y")
    axs[4][2].axis("tight")

    axs[5][0].imshow(
        minv3d_reg[5, :, :].T.get(), cmap="gist_rainbow", vmin=m.min(), vmax=m.max()
    )
    axs[5][0].set_title("Regularized Inverted Model iter x-z")
    axs[5][0].axis("tight")
    axs[5][1].imshow(
        minv3d_reg[:, 200, :].T.get(), cmap="gist_rainbow", vmin=m.min(), vmax=m.max()
    )
    axs[5][1].set_title("Regularized Inverted Model iter y-z")
    axs[5][1].axis("tight")
    axs[5][2].imshow(
        minv3d_reg[:, :, 220].T.get(), cmap="gist_rainbow", vmin=m.min(), vmax=m.max()
    )
    axs[5][2].set_title("Regularized Inverted Model iter x-y")
    axs[5][2].axis("tight")

    axs[6][0].imshow(
        minv3d_admm[5, :, :].T.get(), cmap="gist_rainbow", vmin=m.min(), vmax=m.max()
    )
    axs[6][0].set_title("TV-Regularized Inverted Model iter x-z")
    axs[6][0].axis("tight")
    axs[6][1].imshow(
        minv3d_admm[:, 200, :].T.get(), cmap="gist_rainbow", vmin=m.min(), vmax=m.max()
    )
    axs[6][1].set_title("TV-Regularized Inverted Model iter y-z")
    axs[6][1].axis("tight")
    axs[6][2].imshow(
        minv3d_admm[:, :, 220].T.get(), cmap="gist_rainbow", vmin=m.min(), vmax=m.max()
    )
    axs[6][2].set_title("TV-Regularized Inverted Model iter x-y")
    axs[6][2].axis("tight")

    axs[7][0].imshow(
        minv3d_pd[5, :, :].T.get(), cmap="gist_rainbow", vmin=m.min(), vmax=m.max()
    )
    axs[7][0].set_title("TV-PD Inverted Model iter x-z")
    axs[7][0].axis("tight")
    axs[7][1].imshow(
        minv3d_pd[:, 200, :].T.get(), cmap="gist_rainbow", vmin=m.min(), vmax=m.max()
    )
    axs[7][1].set_title("TV-PD Inverted Model iter y-z")
    axs[7][1].axis("tight")
    axs[7][2].imshow(
        minv3d_pd[:, :, 220].T.get(), cmap="gist_rainbow", vmin=m.min(), vmax=m.max()
    )
