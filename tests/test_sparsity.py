...
from pylops.basicoperators import MatrixMult, BlockDiag, Identity, VStack, HStack
from pylops.optimization.sparsity import ista as ista_pylops, fista as fista_pylops
...
def test_ISTA_FISTA_alpha_too_high(par):
    ...
    x[:] = np.zeros(par["nx"], dtype=par['dtype']) + par["imag"] * np.zeros(par["nx"], dtype=par['dtype'])
    x[par["nx"] // 2] = 1.0 + par["imag"] * 1.0
    y = Aop * x

    if par["x0"]:
        x0 = DistributedArray(global_shape=size * par['nx'], dtype=par['dtype'], engine=backend)
        x0[:] = np.ones(par["nx"], dtype=par['dtype']) + par["imag"] * np.ones(par["nx"], dtype=par['dtype'])
    else:
...
def test_ISTA_FISTA(par):
    ...
    A = A + par["imag"] * A
    Aop = MPIBlockDiag(ops=[MatrixMult(np.asarray(A, dtype=par['dtype']), dtype=par["dtype"])], dtype=par['dtype'])
...
def test_ISTA_FISTA_broadcastmodel(par):
    ...
    np.fill_diagonal(A, d)
    A = A + par["imag"] * A
    Aop = MPIVStack(ops=[MatrixMult(np.asarray(A, dtype=par['dtype']), dtype=par["dtype"])], dtype=par['dtype'])
...
def test_ISTA_FISTA_broadcastdata(par):
    ...
    Aop = MPIHStack(ops=[MatrixMult(np.asarray(A), dtype=par["dtype"])], dtype=par['dtype'])
...
def test_ISTA_FISTA_stopping(par):
    """Invert problem with ISTA/FISTA - Stopping"""
    np.random.seed(10)
    A = np.random.randn(par["ny"], par["nx"]) + par["imag"] * np.random.randn(
        par["ny"], par["nx"]
    )
    ...
    x = DistributedArray(global_shape=size * par['nx'], dtype=par['dtype'], engine=backend)
    x[:] = np.zeros(par["nx"]).astype(par['dtype']) + par["imag"] * np.zeros(par["nx"]).astype(par['dtype'])
    x[par["nx"] // 2] = 1.0 + par["imag"] * 1.0
    x[3] = 1.0 + par["imag"] * 1.0
    x[par["nx"] - 4] = -1.0 - par["imag"] * 1.0
    y = Aop * x
