"""Test the MPIStackedLinearOperator class
    ...
    $ mpiexec -n 10 pytest test_stackedlinearop_nccl.py --with-mpi
...
def test_transpose_nccl(par):
    ...
    # For forward mode
    dist_1 = pylops_mpi.DistributedArray(global_shape=size * par['ny'], base_comm_nccl=nccl_comm, dtype=par['dtype'], engine="cupy")
    dist_1[:] = cp.ones(par['ny'])
    dist_2 = pylops_mpi.DistributedArray(global_shape=par['nx'] * par['ny'], base_comm_nccl=nccl_comm, dtype=par['dtype'], engine="cupy")
    dist_2[:] = cp.ones(dist_2.local_shape)
    ...
    # For adjoint mode
    dist_1 = pylops_mpi.DistributedArray(global_shape=size * par['nx'], base_comm_nccl=nccl_comm, dtype=par['dtype'], engine="cupy")
    dist_1[:] = cp.ones(par['nx'])
    dist_2 = pylops_mpi.DistributedArray(global_shape=par['nx'] * par['ny'], base_comm_nccl=nccl_comm, dtype=par['dtype'], engine="cupy")
    dist_2[:] = cp.ones(dist_2.local_shape)
    ...
    if rank == 0:
        ...
        assert_allclose(Top_x_np.get(), Top @ x_global.get(), rtol=1e-14)
        assert_allclose(Top_y_np.get(), Top.H @ y_global.get(), rtol=1e-14)
...
def test_scaled_nccl(par):
    ...
    if rank == 0:
        ...
        assert_allclose(Sop_x_np.get(), Sop @ x_global.get(), rtol=1e-14)
        assert_allclose(Sop_y_np.get(), Sop.H @ y_global.get(), rtol=1e-14)
...
def test_conj_nccl(par):
    ...
    if rank == 0:
        ...
        assert_allclose(Cop_x_np.get(), Sop @ x_global.get(), rtol=1e-14)
        assert_allclose(Cop_y_np.get(), Sop.H @ y_global.get(), rtol=1e-14)
...
def test_power_nccl(par):
    ...
    if rank == 0:
        ...
        StackedBlockDiag = pylops.BlockDiag([BDiag, FirstDeriv])
        Sop = StackedBlockDiag.conj()
        assert_allclose(Pop_x_np.get(), Sop @ x_global.get(), rtol=1e-14)
        assert_allclose(Pop_y_np.get(), Sop.H @ y_global.get(), rtol=1e-14)
...
def test_sum_nccl(par):
    ...
    dist1[:] = cp.ones(dist1.local_shape)
    dist2 = pylops_mpi.DistributedArray(global_shape=par['ny'] * par['nx'], base_comm_nccl=nccl_comm, dtype=par['dtype'], engine="cupy")
    dist2[:] = cp.ones(dist2.local_shape)
...
def test_product_nccl(par):
    ...
    dist1 = pylops_mpi.DistributedArray(global_shape=size * par['ny'], base_comm_nccl=nccl_comm, dtype=par['dtype'], engine="cupy")
    dist1[:] = cp.ones(dist1.local_shape)
    dist2 = pylops_mpi.DistributedArray(global_shape=size * par['nx'], base_comm_nccl=nccl_comm, dtype=par['dtype'], engine="cupy")
    dist2[:] = cp.ones(dist2.local_shape)
    x = pylops_mpi.StackedDistributedArray(distarrays=[dist1, dist2])
    x_global = x.asarray()
    Pop_x = Pop_MPI @ x
    ...
    if rank == 0:
        ...
        assert_allclose(Pop_x_np.get(), Pop @ x_global.get(), rtol=1e-14)
        assert_allclose(Pop_y_np.get(), Pop.H @ y_global.get(), rtol=1e-14)
